...
from src.data.Dataset import (describe_sitk, split_one_4d_sitk_in_list_of_3d_sitk, get_phases_as_onehot) # , match_hist)
...
class PhaseRegressionGenerator_v2(DataGenerator):
    ...
    def __preprocess_one_image__(self, i, ID):
        ...
        # if self.NNUNET_SEG:
        #     mask_inputs = np.stack([mask_inputs, mask_targets], axis=-1)
...
# class GeneratorForDice(PhaseRegressionGenerator_v2):
...
#             df.columns = df.columns.str.lower()
#             self.DF_METADATA = df[['patient', 'ed#', 'ms#', 'es#', 'pf#', 'md#']]
#             if self.ISACDC: self.DF_METADATA.loc['patient'] = self.DF_METADATA['patient'].str.zfill(3)
#         else:
#             self.DF_METADATA = None
#
#         logging.info('Temporal phase augmentation: \n{}'
...
#             for i, future in enumerate(as_completed(futures)):
#                 zipped.append(future.result())
#             self.model_inputs, self.onehots, self.reps, self.gt_lengths,  self.segmentation = list(map(list, zip(*zipped)))
#
#         self.MASKS = None  # need to check if this is still necessary!
#
#         random.seed(config.get('SEED', 42))
#         logging.info('params of generator:')
#         exclude_vars = ['IMAGES', 'LABELS', 'model_inputs', 'onehots', 'reps', 'gt_lengths', 'config', 'use_segmentation']
#         valid_var_types = [int, str, list, bool]
#         logging.info(list((k, v) for k, v in vars(self).items() if
#                           type(v) in valid_var_types and str(k) not in exclude_vars))
#
...
#
#         :param list_IDs_temp: list of int - list of ids to load
#         :return: x (4d-CMR),y (Phase vector, rolled 4d-CMR, zero-flowfield, use_segmentation)
#         [(batchsize, *dim, n_channels)],
#         [(batchsize, self.T_SHAPE, number_of_classes), (batchsize, *dim, n_channels), (batchsize, *dim, 3), (batchsize, T_shape, *dim, 3]
#         """
#
#         # Initialization
#         x = np.empty_like(self.X_SHAPE)  # model input
#         y2 = np.zeros_like(self.X_ROLLED_SHAPE)  # rolled volume
#         y = np.zeros_like(self.Y_SHAPE)  # model output
#         y_segmentation = np.empty([self.BATCHSIZE, self.T_SHAPE, *self.DIM, self.N_CLASSES])
#
#         futures = set()
#
#         # One thread per worker
...
#             # otherwise slower images will always be at the end of the batch
#             try:
#                 x_, _, _, y3_, i, ID, needed_time = future.result()
#                 x[i,], y_segmentation[i] = x_, y3_
#                 logging.debug('img finished after {:0.3f} sec.'.format(needed_time))
#             except Exception as e:
#                 # write these files into a dedicated error log
#                 PrintException()
...
#         logging.debug('Batchsize: {} preprocessing took: {:0.3f} sec'.format(self.BATCHSIZE, time() - t0))
#
#         return tuple([[x], [y, y2, np.zeros((*y2.shape[:-1], 3)), y_segmentation]])
#
#     def __fix_preprocessing__(self, i, ID):
#
#         t0 = time()
...
#                                        masking_values=self.MASKING_VALUES, replace=self.REPLACE_WILDCARD)
#
#         y_seg = self.LABELS[ID]  # get list of labeled 3d volumes
#         if type(y_seg) == list: # ACDC
#             segmentation_outputs = [load_masked_img(sitk_img_f=y, mask=self.MASKING_IMAGE,
#                                                     masking_values=self.MASKING_VALUES, replace=self.REPLACE_WILDCARD,
#                                                     mask_labels=self.MASK_VALUES)
#                                     for y in y_seg]
#         else: # TOF
#             self.ISACDC = False
#             segmentation_outputs = load_masked_img(sitk_img_f=y_seg, mask=self.MASKING_IMAGE,
#                                                    masking_values=self.MASKING_VALUES, replace=self.REPLACE_WILDCARD,
#                                                    mask_labels=self.MASK_VALUES)
#             segmentation_outputs = split_one_4d_sitk_in_list_of_3d_sitk(segmentation_outputs)
#
#         # Create a list of 3D volumes for resampling
#         model_inputs = split_one_4d_sitk_in_list_of_3d_sitk(model_inputs, axis=0)
#
...
#                 # transform the spacing from numpy representation towards the sitk representation
#
#                 # only resample xy and append zeros to z to match required size
#                 original_spacing = model_inputs[0].GetSpacing()
#                 original_size = model_inputs[0].GetSize()
#
#                 target_spacing = list(reversed(self.SPACING))
#                 target_spacing[-1] = original_spacing[0]
#                 new_size_inputs = list(map(lambda elem: calc_resampled_size(elem, target_spacing), model_inputs))
#             else:
#                 raise NotImplementedError('dimension not supported: {}'.format(model_inputs[0].GetDimension()))
#
...
#                                                 interpolate=self.IMG_INTERPOLATION),
#                                     zip(model_inputs, new_size_inputs)))
#             segmentation_outputs = list(map(lambda x:
#                                             resample_3D(sitk_img=x[0],
#                                                         size=x[1],
#                                                         spacing=target_spacing,
#                                                         interpolate=sitk.sitkNearestNeighbor),
#                                             zip(segmentation_outputs, new_size_inputs)))
#
#         logging.debug('Spacing after resample: {}'.format(model_inputs[0].GetSpacing()))
#         logging.debug('Size after resample: {}'.format(model_inputs[0].GetSize()))
#
#         # transform to nda for further processing
#         model_inputs = np.stack(list(map(lambda x: sitk.GetArrayFromImage(x), model_inputs)), axis=0)
#         segmentation_outputs = np.stack(list(map(lambda x: sitk.GetArrayFromImage(x), segmentation_outputs)), axis=0)
#
#         # flip base/apex for ACDC
#         if self.ISACDC:
#             # before: axis 1 == z-axis: base --> apex, after:  apex--> base
#             model_inputs = np.flip(model_inputs, axis=1)
#             segmentation_outputs = np.flip(segmentation_outputs, axis=1)
#
...
#             model_inputs, segmentation_outputs = align_inplane_with_ip(model_inputs=model_inputs,
#                                                                        msk_file_name='',
#                                                                        mask=np.array(segmentation_outputs),
...
#             segmented_ts = sorted([int(y.split('_frame')[1].split('_gt.nii.gz')[0]) - 1 for y in y_seg])  # ts0 == frame1
#             segmentation_outputs = np.zeros_like(model_inputs)
#             segmentation_outputs[segmented_ts] = _
#
#         # Center-crop z, x, y, keep t
#         # normalise
#         model_inputs = pad_and_crop(model_inputs, target_shape=(model_inputs.shape[0], *self.DIM))
#         segmentation_outputs = pad_and_crop(segmentation_outputs, target_shape=(model_inputs.shape[0], *self.DIM))
#         model_inputs = clip_quantile(model_inputs, .95)  # lower_boundary=-1*np.quantile(-1*model_inputs, .90)
#         model_inputs = normalise_image(model_inputs, normaliser=self.SCALER)  # normalise per 4D
#
#         return model_inputs, None, None, None, segmentation_outputs,
#
#     def __preprocess_one_image__(self, i, ID):
#
#         """
...
#         t0 = time()
#         if self.IN_MEMORY:  # load pre-loaded images from the current gen object
#             model_inputs, segmentation = self.model_inputs[ID], self.segmentation[ID]
#         else:
#             model_inputs, _, _, _, segmentation = self.__fix_preprocessing__(i, ID)
#
#         segmentation = transform_to_binary_mask(segmentation, mask_values=[1, 2, 3])
#
#         # roll
#         # Fake a ring behaviour of the onehot vector by
#         # first, tile along t
...
#         # third split+maximise element-wise on both matrices
#         # we normalise before definition of the target, to make sure both are from the same distribution
#         segmentations = list(range(len(segmentation)))
#         _ = np.zeros((self.T_SHAPE,) + model_inputs.shape[1:])
#         _[segmentations] = model_inputs
#         model_inputs = _
#         _ = np.zeros((self.T_SHAPE,) + segmentation.shape[1:])
#         _[segmentations] = segmentation
#         segmentation = _
#         model_targets = model_inputs
#         # test
#         model_inputs = np.stack([model_inputs, model_targets], axis=-1)
#         # make sure we do not introduce nans to the model
#         assert not np.any(np.isnan(model_inputs))
#         assert not np.any(np.isnan(model_targets))
#         return model_inputs, model_targets[..., None], None, segmentation, i, ID, time() - t0
#
