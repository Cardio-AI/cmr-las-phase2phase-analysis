...
from src.data.Dataset import get_metadata_maybe, copy_meta_and_save
import numpy as np
...
#
#
# def align_inplane_with_ip(model_inputs, msk_file_name, roll2septum=True, roll2lvbood=False, rotate=True,
#                           translate=True, mask=None, return_mask=False):
#     '''
...
#         if len(center==3): center = center[1:]  # ignore z-axis for translation
...
#         if mask is not None and return_mask is not None:
#             mask = np.roll(mask, ry, axis=-2)
#             mask = np.roll(mask, rx, axis=-1)
#         if len(center) == 3:
#             nz = model_inputs.shape[-3]
...
#             if mask is not None and return_mask is not None: mask = np.roll(mask, rz, axis=-3)
#
#     if rotate:
...
#         if mask and return_mask: mask = ndimage.rotate(mask, angle=rot_angle, reshape=False, order=1, axes=(-3, -2))
#
#     if mask is not None and return_mask is not None:
...
#     lower_gray_percentile_threshold = .90
#     mse_threshold = 90
#     gaus_sigma = 3
#     nyx = np.array([int(ny), int(nx)])
#     border = ((nyx / 100) * border_percentage).astype(np.int64)
...
#     temp = normalise_image(temp[:-1, border[0]:-border[0], border[1]:-border[1]], normaliser='standard')
...
#         return np.array(largestCC)
#
#     mse_mean_mask = mse_mean > np.percentile(mse_mean, mse_threshold)
#     mse_mean_mask_filter = getLargestCC(mse_mean_mask)
#     # if the largest CC is less than 70% of the whole mask, we probably have two or more 'big' components
#     # In this case we dont know if the biggest CC is the ventricle or an artefact
...
#         if reduced_part < 0.7:
#             # this is a strong indicator that there are more than two possible focuses, better use the center
#             center = np.array(mse_mean_mask_filter.shape) // 2  # center of volume
#         # center = np.array(center[1:]) + border
#         center = np.array([center[0], center[1] + border[0], center[2] + border[1]])
#     except Exception as e:
#         print('centers')
#         print(e)
#         center = np.array(mse_mean_mask_filter.shape) // 2  # center of volume
#     # current experiments indicate that for the z-axis it is better to take the volume center
#     return center[1:]
#
#
...
#     return np.array([f_z, f_y + border[0], f_x + border[1]])
#
#
