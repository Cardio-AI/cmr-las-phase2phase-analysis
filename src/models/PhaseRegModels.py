...
class PhaseRegressionModel:
    ...
    def __init__(self, config, networkname='PhaseRegressionModel'):
        ...
        with self.strategy.scope():
            ...
            self.input_shape = config.get('DIM', [1, 224, 224])
            ...
            self.Conv_layer = self.Conv(self.ndims, kernel_size=3, padding='same',
                                        kernel_initializer=tensorflow.keras.initializers.RandomNormal(mean=0.0,
                                                                                                      stddev=1e-5),
                                        name='unet2flow')
            ...
            self.enc = keras.Model(inputs=self.unet.inputs,
                                   outputs=[self.unet.layers[(len(self.unet.layers) // 2) - 1].output])
            self.st_layer = nrn_layers.SpatialTransformer(interp_method=self.interp_method, indexing=self.indexing,
                                                          ident=True,
            ...
            def create_seg_model(model_path, weights_path):
                ...
                model.compile(optimizer='adam', loss='BcdDiceLoss', metrics=[keras.metrics.binary_accuracy])
                model.trainable = False
                return model

            if self.PRETRAINED_SEG:
                segmentation_model = create_seg_model(self.config['SEGMENTATION_MODEL'], self.config['SEGMENTATION_WEIGHTS'])
            ...
            backward_layer = LSTM(self.lstm_units, return_sequences=True, dropout=0.0, go_backwards=True,
                                  name='backward_LSTM')
            self.bi_lstm_layer = Bidirectional(forward_layer, backward_layer=backward_layer, merge_mode='ave',
                                               name='biLSTM')

            forward_layer1 = LSTM(self.lstm_units, return_sequences=True, dropout=0.0, name='forward_LSTM1')
            backward_layer1 = LSTM(self.lstm_units, return_sequences=True, dropout=0.0, go_backwards=True,
                                   name='backward_LSTM1')
            self.bi_lstm_layer1 = Bidirectional(forward_layer1, backward_layer=backward_layer1, merge_mode='ave',
                                                name='biLSTM1')
    ...
    def get_model(self):
        ...
        with self.strategy.scope():
            ...
            transformed = TimeDistributed(self.st_lambda_layer, name='4d-p2p-st')(
                keras.layers.Concatenate(axis=-1)([self.input_tensor[..., 0:1], flows]))
            ...
            if self.downsample_flow_features:
                ...
                if self.split_corners:  # average per corner per 3D volume, e.g.: top left, top right ...
                    ...
                    flow_features = keras.layers.Concatenate(axis=-1, name='split_corners')(
            ...
            if self.add_bilstm:
                ...
                minmax_lambda_tf = keras.layers.Lambda(lambda x:
                                                       (x - tf.reduce_min(x)) / (
                                                               tf.reduce_max(x) - tf.reduce_min(
                                                           x) + keras.backend.epsilon()),
                                                       name='minmaxscaling')
            ...
            model.compile(
                ...
                metrics={
                    'transformed': [own_metr.MSE(), own_metr.SSIM()],
                }
            )
