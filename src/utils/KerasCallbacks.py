...
def get_callbacks(config=None, batch_generator=None, validation_generator=None, metrics=None):
    ...
    # callbacks.append(
    #     SegmentationImageSaver(log_dir=config['TENSORBOARD_PATH'],
    #                            image_freq=config.get('SAVE_LEARNING_PROGRESS_FREQUENCY', 2),
    #                            feed_inputs_4_display=feed_inputs_4_tensorboard(config, batch_generator,
    #                                                                            validation_generator))
    # )
...
def feed_inputs_4_tensorboard(config, batch_generator=None, validation_generator=None, samples=4):
    ...
    if batch_generator is not None:
        ...
        if y_t is not None:
            x_t, y_t = x_t[0][0:samples], y_t[1][0:samples]
        else:
    ...
    if validation_generator is not None:
        ...
        x_v, y_v = x_v[0][0:samples], y_v[1][0:samples]
                ...
                for key, x, y in zip(self.keys, self.xs, self.ys):
                    ...
                    for p in range(len(phases)):
                        ...
                        tensorflow.summary.image(
                            name='plot/{}/batch_{}/{}_{}/summary'.format(key, elem_in_b, p, phases[p]),
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.e % self.freq == 0 or self.e < self.n_start_epochs:  # every n epoch (and the first 20 epochs), write pred in a TensorBorad summary file;
            ...
            for key, x, y in zip(self.keys, self.xs, self.ys):
                ...
                x = [x[..., 0:1]]
                if len(predictions) == 4:  # multi-output-model
                    onehot_predictions, movings, vects, segmentation = predictions
                elif len(predictions) == 3:
                    onehot_predictions, movings, vects = predictions
                    onehot_y = y[0]  # y is a list with [onehot, moved, zeros, zeros], we are interested in the onehot
                    ...
                    for b in range(
                            min(gt.shape[0], upper_plot_limit)):  # set an upper limit, to save time and disk space
                        ...
                        for t_idx, t in enumerate(
                                ind_gt):  # iterate over the 5 gt phases to plot the volume at these timesteps
                            first_vol, second_vol = x[0][b][t], y[b][t]  # extract the cmr per phase
                            ...
                            with self.writer.as_default():
                                tensorflow.summary.image(
                                    name='plot/{}/batch_{}/{}_{}/summary'.format(key, b, t, self.phases[t_idx]),
                                    data=self.make_image(fig),
                                    step=epoch)
                    ...
                    with self.writer.as_default():
                        tensorflow.summary.image(name='plot/{}/_pred'.format(key, pred_i),
                                                 data=self.make_image(show_phases(onehot_y[:upper_plot_limit],
                                                                                  onehot_predictions[
                                                                                  :upper_plot_limit])),
                                                 step=epoch)


class SegmentationImageSaver(Callback):
    # Keras Callback for saving use_segmentation in the tensorboard

    def __init__(self, log_dir='', image_freq=2, feed_inputs_4_display=None, dpi=200, f_size=(5, 5),
                 interpol='bilinear'):
        ...
        self.epoch = 0
        self.feed_inputs_4_display = feed_inputs_4_display
        self.image_dir = os.path.join(log_dir, 'images')  # create a subdir
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for key, x, y in zip(self.keys, self.xs, self.ys):
            if key == 'gen_train':
                continue  # TODO: why has gen_train different shape for y?
            predictions = self.model.predict(x)
            # remove the target from x, we dont need it for plotting
            if len(predictions) == 4:  # multi-output-model
                onehot, movings, vects, segmentation = predictions
                input_volume = x[0]  # get input image
                gt = y[-1]

                for b in range(input_volume.shape[0]):  # set an upper limit, to save time and disk space
                    t = 0
                    # create fig
                    fig, axes = plt.subplots(3, 2)
                    fig.suptitle('Segmentation')
                    for i, z in enumerate([15, 8, 0]): # Todo make more generic
                        show_slice(img=input_volume[b, t, z, :, :, 1], mask=segmentation[b, t, z],
                                   show=False, f_size=self.f_size, dpi=self.dpi, interpol=self.interpol,
                                   cmap='gray', ax=axes[i, 0])
                        show_slice(img=input_volume[b, t, z, :, :, 1], mask=gt[b, t, z],
                                   show=False, f_size=self.f_size, dpi=self.dpi, interpol=self.interpol,
                                   cmap='gray', ax=axes[i, 1])

                    axes[0, 0].set_title('Prediction')
                    axes[0, 1].set_title('GT')
                    axes[0, 0].set_ylabel('Base')
                    axes[1, 0].set_ylabel('Mid')
                    axes[2, 0].set_ylabel('Apex')
                    plt.tight_layout()
                    with self.writer.as_default():
                        tensorflow.summary.image(
                            name=f'plot/{key}/batch_{b}/t_{t}/summary',
                            data=self.make_image(fig),
                            step=epoch)
        self.epoch += 1
